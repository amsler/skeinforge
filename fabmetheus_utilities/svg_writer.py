...
#		translate = (-1 * minX) (-1 * minY)
	...
	def addLayerEnd( self, rotatedBoundaryLayer ):
		...
		pathString = '\t\t\t<path transform="scale(%s, %s) translate(%s, %s)" d="' % ( self.unitScale, - self.unitScale, cornerMinimumXString, cornerMinimumYString )
	...
	def getInitializationForOutputSVG( self, procedureName ):
		...
		canvasInitializationOutput.write( '\tlayerThickness = %s\n' % self.getRounded( self.carving.getCarveLayerThickness() ) ) # Set layer thickness.
