...
class ChopRepository:
	...
	def __init__( self ):
		...
		self.fileNameInput = settings.FileNameInput().getFromFileName( skeinforge_interpret.getTranslatorFileTypeTuples(), 'Open File to be Chopped', self, '' )
...
class ChopSkein( svg_codec.SVGCodecSkein ):
	...
	def getCarvedSVG( self, carving, fileName, repository ):
		...
		return self.getReplacedSVGTemplate( fileName, 'chop', svg_codec.getTruncatedRotatedBoundaryLayers( repository, rotatedBoundaryLayers ) )
