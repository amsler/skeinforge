...
The following examples shows functions of skeinforge_interpret.  The examples are run in a terminal in the folder which contains skeinforge_interpret.py.
...
>>> import interpret
>>> skeinforge_interpret.getGNUTranslatorGcodeFileTypeTuples()
[('GTS files', '*.gts'), ('Gcode text files', '*.gcode'), ('STL files', '*.stl'), ('SVG files', '*.svg')]

>>> skeinforge_interpret.getImportPluginFileNames()
['gts', 'stl', 'svg']
...
def getPluginsDirectoryPath():
	...
	return gcodec.getAbsoluteFolderPath( os.path.dirname( __file__ ), os.path.join( os.path.join( 'skeinforge_tools', 'meta_plugins' ), 'interpret_plugins' ) )
...
def interpretFile( fileName ):
	...
	startTime = time.time()
	interpretSkein = InterpretSkein()
	interpretGcode = interpretSkein.getInterpretation( fileName )
	if interpretGcode == '':
		return
	suffixFileName = fileName[ : fileName.rfind( '.' ) ] + '_interpret.' + interpretSkein.suffix
...
class InterpretRepository:
	...
	def __init__( self ):
		...
		settings.addListsToRepository( 'skeinforge_tools.meta_plugins.interpret.html', '', self )
...
class InterpretSkein:
	"A class to interpret a carving."
	def getInterpretation( self, fileName ):
		"Parse file and store the interpret gcode."
		pluginModule = getInterpretPlugin( fileName )
		if pluginModule == None:
			return ''
		gcodeText = str( pluginModule.getCarving( fileName ) )
		self.suffix = pluginModule.getInterpretationSuffix()
		return gcodeText
